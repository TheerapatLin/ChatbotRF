...
- จงวิเคราะห์โฟลเดอร์ backend ในส่วนของ API POST`/api/audio/tts` แล้วสร้างไฟล์ HTML ง่ายๆสำหรับการทดสอบ API นี้ โดยมีเงื่อนไขดังต่อไปนี้ - มีช่อง input ใส่ข้อความด้านล่างสุด และปุ่มกดเพื่อส่งข้อความให้ API - response ที่ได้จาก API ที่เป็นเสียงจะถูกเล่นทันที - ขณะที่เสียงกำลังเล่นอยู่ สามารถกดหยุดเสียงกลางคันได้